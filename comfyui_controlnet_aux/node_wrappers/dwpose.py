...
if not os.environ.get("DWPOSE_ONNXRT_CHECKED"):
    if check_ort_gpu():
        # Only print if verbose logging is enabled
        if os.environ.get("COMFYUI_VERBOSE", "0") == "1":
            print("DWPose: Onnxruntime with acceleration providers detected")
    else:
        # Only warn if verbose logging is enabled or if this is the first time
        if os.environ.get("COMFYUI_VERBOSE", "0") == "1" or os.environ.get("DWPOSE_WARNED", "0") == "0":
            warnings.warn("DWPose: Onnxruntime not found or doesn't come with acceleration providers, switch to OpenCV with CPU device. DWPose might run very slowly")
            os.environ['DWPOSE_WARNED'] = '1'
