...
try:
    ...
    # Fallback if ComfyUI not available during import
    comfy = None
try:
    from aiohttp import web
    from server import PromptServer
except ImportError:
    # Fallback if server not available during import
    web = None
    PromptServer = None
...
original_calculate_sigmas = None
if comfy:
...
def init(check_imports=None):
    # Only show init message in debug mode
    RESplain("Init", debug=True)
    ...
    # Only proceed with ComfyUI integration if comfy is available
    if not comfy:
        RESplain("ComfyUI not available during init, skipping integration", debug=True)
        return True
    ...
    if using_RES4LYF_time_snr_shift:
        ...
        RESplain("Using RES4LYF time SNR shift but this is deprecated and will be disabled at some completely unpredictable point in the future", debug=True)
...
def install_js():
    ...
    if not os.path.exists(src_dir):
        RESplain("No JS", debug=True)
        return

    should_install = should_install_js()
    if should_install:
        RESplain("it looks like you're running an old version of ComfyUI that requires manual setup of web files, it is recommended you update your installation.", "warning")
    dst_dir = get_web_ext_dir()
    linked = os.path.islink(dst_dir) or is_junction(dst_dir)
    if linked or os.path.exists(dst_dir):
        if linked:
            if should_install:
                RESplain("JS already linked", debug=True)
            else:
                os.unlink(dst_dir)
                RESplain("JS unlinked, PromptServer will serve extension", debug=True)
        elif not should_install:
            shutil.rmtree(dst_dir)
            RESplain("JS deleted, PromptServer will serve extension", debug=True)
        return
    
    if not should_install:
        RESplain("JS skipped, PromptServer will serve extension", debug=True)
        return
    
    if link_js(src_dir, dst_dir):
        RESplain("JS linked", debug=True)
        return

    RESplain("Copying JS files", debug=True)
